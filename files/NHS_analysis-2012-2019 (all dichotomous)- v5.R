...
# BACKWARD
## NOT LAGGED
lm.out1 <- lm(SHMI.Index ~ Engagement.Index + RecommendWork.Index + WorkStress.Index,  weights = Survey.clinical.proportion*Responses, data = data.NHS.2012_2019 )
summary(lm.out1)
...
## Spline analysis-------------------------------
# In version 5, switched spline to means rather than dichot
...
##* determine optimal number of knots CROSS-LAGGED (01/07/2021 - who cares) -----
#--
xlab <- "Engagement"
data.temp$factor <- data.temp$Engagement.Index.dichot
xlab <- "Recommend Work"
...
##* Multivariable spline with cross-lag 2021-01-03----
...
##  + SHMI.before restored to the model 01/07/2021
...
# Dropping engagement and workstress
...
plot(data.temp$Engagement.Index.dichot,data.temp$WorkStress.Index,xlab='Engagement',ylab='Work stress (rate)',main=Title)
...
plot(data.temp$Engagement.Index.dichot,data.temp$RecommendWork.Index.dichot,xlab='Engagement (rate)',ylab='Work stress (rate)',main=Title)
#text(table.temp$Engagement.Index,table.temp$WorkStress.Index,paste(table.temp$Year.Index,'',sep=''),pos=3)
xmin <- par("usr")[1] + strwidth("A")
xmax <- par("usr")[2] - strwidth("A")
ymin <- par("usr")[3] + 1.2* strheight("A")
...
lm.out1 <- lm(RecommendWork.Index.dichot ~ Engagement.Index.dichot,  weights = NULL, data = table.temp )
...
text(xmin,ymax-1.1* strheight("A"),adj=c(0,1),paste("R2 (adjusted) = ",R2,sep=))
